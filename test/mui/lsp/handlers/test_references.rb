...
class TestReferencesHandler < Minitest::Test
  def setup
    @editor = MockEditor.new
    @client = MockClient.new
    @handler = Mui::Lsp::Handlers::References.new(editor: @editor, client: @client)
  end

  class TestHandleResult < TestReferencesHandler
    ...
    def test_handle_result_with_multiple_locations
      ...
      assert_includes @editor.scratch_buffer_content, "References (3 found)"
      assert_includes @editor.scratch_buffer_content, "file1.rb:11"
      assert_includes @editor.scratch_buffer_content, "file2.rb:21"
      assert_includes @editor.scratch_buffer_content, "file3.rb:31"
    end

    def test_handle_result_with_empty_result
      @handler.handle([], nil)

      assert_equal "No references found", @editor.message
    end
    ...
    def test_stores_locations_for_picker
      ...
      assert_equal :references, @editor.instance_variable_get(:@lsp_picker_type)
    end
  end

  class TestHandleError < TestReferencesHandler
